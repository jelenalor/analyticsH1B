...
def createMultiGraph(items, y_click):
    # top_companies = df[df.city ==city].groupby("company").count()["job_title"].sort_values(ascending=False)[:20].index.tolist()
    # dff = df[df.company.isin(top_companies)].drop(["state", "city"], axis=1)
    # dff_gp = dff.groupby("company").agg( \
    #     {"job_title": "count", "base_salary": salary_choice} \
    #     ).rename(columns={"job_title": "count"}).sort_values(by="count", ascending=False)
    ...
    # y_count_click = y_count
    # x_li_click = x
    # y_click = y_count.pop(x.index(y_click))
    # x_click = x.pop(x.index(y_click))
...
def dataForTopRoles(city, salary_choice):
    ...
    y_choice = [tokens_summary[i]["all"][1] if
                salary_choice == "mean" else tokens_summary[i]["all"][2]
                if salary_choice == "min" else tokens_summary[i]["all"][3]
                for i in x]
    return city, salary_choice, x, y_count, y_choice
